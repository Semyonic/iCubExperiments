//
//
//  Created by Semih Onay on 16.03.2015.
//
//
...
public:
	...
	virtual bool loadParams() {
		...
		robotname = rf.check("robot", Value("icub_Sim"),"robot name").asString().c_str();
	...
	virtual bool threadInit() {
		...
		// set trajectory time
		carm->setTrajTime(trajTime);//slow for better observation
		...
		while (isStopping() != true) {
			...
			if (bfL.size() == 2 && bfR.size() == 2) {
				...
				if (posResp == "yes" || posResp == "y") {
					...
					//move to hover the hand over the XY position of the target: 
					//[X, Y, Z=0.2], with palm upright
					objPos[2] = 0.1;
